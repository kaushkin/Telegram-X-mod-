...
public class GhostSettingsController extends ViewController<Void> implements View.OnClickListener {
    ...
    protected View onCreateView(Context context) {
        ...
        items.add(new ListItem(ListItem.TYPE_HEADER, 0, 0, "Сохранение удаленных"));
        items.add(new ListItem(ListItem.TYPE_SHADOW_TOP));
        
        // Enable Ghost Toggle
        items.add(new ListItem(ListItem.TYPE_RADIO_SETTING, ID_ENABLE_GHOST, R.drawable.baseline_visibility_24, "Сохранять удаленные сообщения"));
        
        items.add(new ListItem(ListItem.TYPE_SHADOW_BOTTOM));
        items.add(new ListItem(ListItem.TYPE_DESCRIPTION, 0, 0, "При включении, удаленные сообщения будут сохраняться локально и отмечаться как призраки"));
        
        // Clear section
        items.add(new ListItem(ListItem.TYPE_SHADOW_TOP));
        items.add(new ListItem(ListItem.TYPE_SETTING, ID_CLEAR_GHOSTS, R.drawable.baseline_delete_forever_24, "Очистить историю призраков"));
        items.add(new ListItem(ListItem.TYPE_SHADOW_BOTTOM));
    ...
    public void onClick(View v) {
        ...
        if (id == ID_ENABLE_GHOST) {
            ...
            UI.showToast("История призраков очищена", Toast.LENGTH_SHORT);
        }
    }
}
